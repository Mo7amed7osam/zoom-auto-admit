...
enum ParticipantsCheck {
    ...
    static func run() {
        ...
        // Live read of the participant list, to confirm the parser against the
        // real tree rather than only against fixtures.
        let readout = ZoomAXSupport.participantsReadout(pid: process.pid)
        log.write("Participants readout: available=\(readout.listAvailable) admitted=\(readout.admitted.count) waiting=\(readout.waiting.count) reportedCount=\(readout.reportedCount.map(String.init) ?? "none")")
        for row in readout.admitted {
            log.write("  admitted: name=\(String(reflecting: row.displayName)) roles=\(row.roles.map(\.rawValue).sorted()) raw=\(String(reflecting: row.rawText))")
        }
        for row in readout.waiting {
            log.write("  waiting: name=\(String(reflecting: row.displayName))")
        }
...
/// Starts an instant meeting, opens the Participants panel, and dumps the
/// participant hierarchy.
///
/// Discovery tool for attendance tracking: the participant rows only exist
/// while a meeting is running with the panel open, so the structure cannot be
/// inspected any other way. It reuses the production automation rather than a
/// parallel implementation, so what it captures is what the recorder will see.
/// It never ends the meeting.
enum ParticipantsCapture {
    ...
    static func run(startMeeting: Bool) {
        var lines: [String] = ["Participants capture \(ISO8601DateFormatter().string(from: Date()))"]
        defer { write(lines) }
        ...
        let process = ZoomProcess(pid: zoom.pid, bundleIdentifier: zoom.bundleIdentifier)
        let automation = LiveZoomAutomation()
        ...
        if startMeeting, !meetingRunning(pid: zoom.pid) {
            NSRunningApplication(processIdentifier: zoom.pid)?.activate()
            Thread.sleep(forTimeInterval: 1.0)
            guard let reading = ZoomAXSupport.zoomMenuBarReading(pid: zoom.pid) else {
                lines.append("Zoom's menu bar could not be read.")
                return
            }
            lines.append("Start meeting: \(ZoomAXSupport.pressApplicationMenuItem(titled: "Start meeting", in: reading))")
            lines.append(contentsOf: handlePreJoin(pid: zoom.pid))
        }

        // Wait for the meeting itself.
        var running = false
        for _ in 0..<40 {
            if meetingRunning(pid: zoom.pid) { running = true; break }
            Thread.sleep(forTimeInterval: 1.0)
        }
        lines.append("Meeting menu present: \(running)")
        guard running else {
            lines.append("No meeting started; nothing to capture.")
            return
        }

        lines.append("Participants panel before: \(automation.participantsPanelState(for: process).rawValue)")
        switch automation.openParticipantsPanel(for: process) {
        case .pressed: lines.append("Participants panel: opened (or already open)")
        case .rejected(let reason): lines.append("Participants panel: could not open — \(reason)")
        }
        Thread.sleep(forTimeInterval: 2.5)
        lines.append("Participants panel after: \(automation.participantsPanelState(for: process).rawValue)")

        // Dump every window that is not the home window.
        let application = ZoomAXSupport.freshZoomApplicationElement(pid: zoom.pid, messagingTimeout: 8)
        let windows = ZoomAXSupport.windowsResult(of: application)
        lines.append("")
        ...
        for (index, window) in windows.windows.enumerated() {
            ...
            let snapshot = ZoomAXSupport.snapshot(
                from: ZoomAXSupport.buildTree(from: window, maxDepth: 16, maxChildren: 200, maxNodes: 12_000)
            )
            lines.append("")
            lines.append("===== window[\(index)] title=\(String(reflecting: title)) =====")
            lines.append("meeting structure: \(ZoomAXSupport.hasMeetingStructure(inSnapshot: snapshot))")
            lines.append("")
            lines.append(contentsOf: dump(snapshot, depth: 0, path: "root"))
        }

        lines.append("")
        lines.append("The meeting was left running; it is never ended automatically.")
    }

    private static func meetingRunning(pid: pid_t) -> Bool {
        guard let reading = ZoomAXSupport.zoomMenuBarReading(pid: pid) else { return false }
        return ZoomAXSupport.meetingMenuPresent(inMenuBar: reading.root)
    }

    /// Mirrors the production pre-join step so the capture reaches the meeting.
    private static func handlePreJoin(pid: pid_t) -> [String] {
        var lines: [String] = []
        for _ in 0..<20 {
            guard let reading = ZoomAXSupport.preJoinReading(pid: pid) else {
                Thread.sleep(forTimeInterval: 1.0)
                continue
            }
            lines.append("Pre-join preview: \(String(reflecting: reading.preview.windowTitle))")

            for kind in [ZoomAXSupport.PreJoinControlKind.microphone, .camera] {
                guard let control = reading.preview.control(for: kind) else { continue }
                lines.append("  \(kind.displayName): \(control.state.rawValue)")
                guard control.state == .on else { continue }
                let outcome = ZoomAXSupport.pressPreJoinControl(control, in: reading)
                lines.append("  \(kind.displayName) press: \(outcome)")
                Thread.sleep(forTimeInterval: 0.6)
            }

            if let fresh = ZoomAXSupport.preJoinReading(pid: pid), let start = fresh.preview.start {
                lines.append("  Start press: \(ZoomAXSupport.pressPreJoinStart(start, in: fresh))")
            }
            return lines
        }
        lines.append("No pre-join preview appeared.")
        return lines
    }
